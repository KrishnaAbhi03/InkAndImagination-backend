...
exports.createOrder = async (req, res, next) => {
  try {
    const { items, customerName, customerEmail, address, phone, totalAmount, notes } = req.body;
    ...
    for (let item of items) {
      ...
      if (artwork.stock < item.quantity) {
        return res.status(400).json({
          success: false,
          error: `Insufficient stock for ${artwork.title}. Available: ${artwork.stock}`
        });
      }
    }
    ...
    // Populate order with artwork details
    await order.populate('items.artworkId');
